...
import {
  ...
  RenderCellNames,
  RenderCellUserName,
} from '../chatbot-table-row';
import { HIDE_COLUMNS_TOGGLABLE, handlePermission } from 'src/common/Common';
import { DeleteUser, GetAllUser } from 'src/api/user';
import ChatbotTableToolbar from '../chatbot-table-toolbar';
import { DeleteChatbot, GetAllChatbot } from 'src/api/chatbot';


// ----------------------------------------------------------------------

export default function ChatbotListView() {
  ...
  const fetchdata = async () => {
    const parameter = {
      ...
      Limit: pageSize,
      PageNumber: page,
      Field: sorting?.length > 0 ? sorting?.[0]?.field : "EmployeeMasterID",
      Sort: sorting?.length > 0 ? sorting?.[0]?.sort : "Desc",
    };

    const { result, isLoading } = await GetAllChatbot(parameter);
    setTotalCount(result?.TotalRows)
    const newTableData = result?.data?.map((item) => ({
      ...item,
      id: item?.EmployeeMasterID
    })) || [];
  ...
  const handleDeleteRow = useCallback(
    async (id) => {
      let Params = {
        EmployeeMasterID: selectedEmployee
      }
      const { status, message } = await DeleteChatbot(Params);
      if (status) {
    ...
    [enqueueSnackbar, tableData, selectedEmployee]
  );

  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.chatbot.edit(id));
    },
    [router]
  );
        ...
        const actions = [
          permission?.IsEdit && (
            <GridActionsCellItem
              ...
              label="Edit"
              onClick={() => handleEditRow(params.row.EmployeeMasterID)}
            />
          ),
          permission?.IsDelete && (
            <GridActionsCellItem
              ...
              label="Delete"
              onClick={() => {
                setSelectedEmployee(params.row.EmployeeMasterID);
                confirmRows.onTrue();
              }}
              sx={{ color: 'error.main' }}
            />
